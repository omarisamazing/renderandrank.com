...
- **Admin login color-block moment** (`functions/admin/index.ts`): split card with a lime brand panel (wordmark + takeaway) beside the password form ("Welcome back"); flex-wrap stacking on narrow screens, all login semantics unchanged. Verified via screenshot + computed-style wrap check + live login; typecheck 0 errors.
