...
- **Pricing cards now answer "which is right for me?"** (`src/components/pricing/PricingTabs.tsx`, 21st-inspired, DESIGN.md-strict): tier `badge` values render as lime pills on non-popular cards (popular keeps the black "Most chosen" pill), the `idealFor` field from `src/data/pricing.ts` renders as an "Ideal for:" line under each description, the "Everything in …" lead feature renders semibold, the toggle gains an `aria-label="Billing period"` plus a "Start with cleanup, keep the gains with a retainer" helper line, and popular cards expose an accessible name. No new colors, no shadows — hairlines, pills, and existing tokens only.
