...
- **Voice double-talk fix + idle bar removed** (`src/lib/voiceSession.ts`, `src/components/ChatWidget.astro`): session-level auto-reconnect deleted — its silent revival raced the widget's explicit restart, yielding two simultaneous Live sessions (the reported double voice). Widget is the single restart owner (fresh sessions defensively stop any prior one; liveness flag clears on error/idle). The "Voice off / Start talking" idle bar is gone: stops/drops hide the bar, focus returns to the composer mic, errors keep message + Reconnect. Verified live (no idle bar, error + Reconnect, mic retap retries, token mint 200, screenshot); typecheck 0 errors.
