...
- **Hero geo-grid gains click-to-inspect point popup + metric list** (`src/components/GeoGridPanel.astro`): dots are now buttons (hover/focus scale, per-point `aria-label`) feeding one shared popup card — keyword eyebrow, `Rank X · Y mi` title, Star rank + Clock distance rows, and `Get this coverage` / `Keep exploring` actions (MarkerPopup pattern, no photo/LCP cost). A 3-row metric list (top-3 points, mean rank, blind spots with lime trend pills) swaps with the Today / With-us toggle via the same `data-before`/`data-after` path; popup stays in sync on toggle, closes on Escape/close. No new deps — native Motion `animate`, existing DESIGN.md tokens, `prefers-reduced-motion` gated.
