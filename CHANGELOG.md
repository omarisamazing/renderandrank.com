...
- **Migration 0005 closed (UNCOMMITTED)**: verified `messages.channel` present on both local and remote D1 (no pending migrations) and cleared the stale PROGRESS entries. (Per-post OG covers were prototyped with Fuuld outlines + Lucide glyphs, then removed per owner call — single shared `og-cover.png` stands.)
