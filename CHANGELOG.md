...
- **Hero geo-grid gains a Live-map view (MapLibre)** (`src/components/ui/map.tsx`, `src/components/GeoLiveMap.tsx`, `src/components/GeoGridPanel.astro`, `public/_headers`): the MapLibre primitive is integrated as a proper UI module (`Map`, `MapMarker`, `MarkerContent`, `MarkerTooltip`, `MarkerPopup`, `MarkerLabel`) adapted for this stack — light-only CARTO basemap (no dark sniffing; the site forces `color-scheme: light`), SSR-safe marker/popup creation (effects only, never `document` during render), shared `cn` from `@/lib/utils`, ink/canvas loader tokens, `scrollZoom: false` so the hero never traps page scroll, plus the missing `MarkerPopup` the demo needed. `GeoLiveMap` (`client:visible`) centers downtown Austin with the business pin + 7 audit-point markers (band colors match the grid, hover tooltips, click-to-open rank cards with a `Get this coverage` CTA). The panel gets a Rank grid / Live map toggle; the grid is restored to its clean form (the click-popup + metric list experiment is reverted — it crowded the panel). CSP `connect-src` allows `https://basemaps.cartocdn.com`. The map chunk is lazy (only loads when the island hydrates).
