...
- **Loading speed pass** (`src/layouts/Layout.astro`, `src/pages/book-a-call.astro`, `src/components/Navbar.astro`, `public/og-cover.jpg`): Cal.com embed + preconnect gated to `/book-a-call` only (zero Cal refs on all other pages); `og-cover.png` 1770 KB → 25 KB landscape JPEG (blurred-fill portrait; a naive center-crop decapitated the subject — rejected on sight); `MobileNav` hydrates only on mobile viewports (`client:media`); `index.html` audited (209 KB, overwhelmingly real content — no trim justified). Vitals homepage before→after: FCP 532→476ms, LCP 1516→1456ms, CLS 0 (local stack; structural third-party removal is the real win).
