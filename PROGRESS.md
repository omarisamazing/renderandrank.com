...
- **Pricing cards 21st pass (DESIGN.md-strict, unpushed)**: `PricingTabs.tsx` now renders tier `badge` as lime pills, `idealFor` as an "Ideal for:" selector line, semibold "Everything in …" lead features, a labelled billing-period toggle with helper copy, and accessible popular-card names. `typecheck` 0 errors, `astro build` 15 pages clean. Docs updated (`ARCHITECTURE.md`, `CHANGELOG.md`). Committed locally, NOT pushed per user instruction.
