...
- **Hero geo-grid revamp (popup + metrics, no heavy deps)**: `src/components/GeoGridPanel.astro` now borrows the MarkerPopup pattern (clickable dots → shared card with keyword, rank + distance, audit date, CTA) and the hexagon-card metric pattern (3-row trend list swapping with Today/With-us) using only existing `motion` + `@lucide/astro` + DESIGN.md tokens. Dots are buttons with `aria-label`s, pin carries a "Your business" MarkerLabel, popup syncs on toggle and closes on Escape. `npm run typecheck` 0 errors, `npm run build` 15 pages clean. Docs updated (`CHANGELOG.md`).
